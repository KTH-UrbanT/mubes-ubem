...
""" This example uses FMPy as the environment to handle FMU simulation. It deals only with
...
def getPathList(config):
    ...
    if not os.path.exists(congifPath):
            print('Sorry, the folder '+congifPath+' does not exist... use -Case or -yml option or change your localConfig.yml')
            sys.exit()
    fmufound = False
...
if __name__ == '__main__' :
    ...
    if CaseNameArg:
        ...
        if not os.path.isfile(os.path.join(CaseFolder1, 'ConfigFile.yml')) and not os.path.isfile(os.path.join(CaseFolder2, 'ConfigFile.yml')):
            ...
            try: localConfig =  setConfig.read_yaml(os.path.join(CaseFolder1, 'ConfigFile.yml'))
            except: localConfig =  setConfig.read_yaml(os.path.join(CaseFolder2, 'ConfigFile.yml'))
