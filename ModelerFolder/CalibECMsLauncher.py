...
# ZoneOfInterest = 'String'             #Text file with Building's ID that are to be considered withoin the BuildNum list, if '' than all building in BuildNum will be considered
    ...
    with open('Ham2Simu4Calib_Last.txt') as f: #'Ham2Simu4Calib_Last.txt') as f:
        FileLines = f.readlines()
    Bld2Sim = []
    for line in FileLines:
        Bld2Sim.append(int(line))

    CaseName = 'ECMsYearlyBasis'
    BuildNum = []#Bld2Sim[26:]
    ...
    BoundLim = [[0,1],[0,9],[0.05,0.7],[0.2,4],[15,27],[0.2,2],[0.5,4],[0,0.9],[0,0.9]]
    NbRuns = 100
    CPUusage = 0.8
    SepThreads = True
    CreateFMU = False
...
######################################################################################################################
    ...
    if not BuildNum2Launch:
        ...
        with open(os.path.join(os.path.dirname(os.path.dirname(CurrentPath)),'MUBES_SimResults',CalibBasis, 'GlobalMatchedParam.pickle'), 'rb') as handle:
            GlobalMatch = pickle.load(handle)
        ...
        for idx,nbBuild in enumerate(BuildNum2Launch):
            ...
            if idx < len(DataBaseInput['Build']) and nbBuild in GlobalMatch[CalibBasis].keys():
                ...
                ParamSample= []
                for line in range(len(GlobalMatch[CalibBasis][nbBuild]['AirRecovEff'])):
                    ParamSample.append([GlobalMatch[CalibBasis][nbBuild][name][line] for name in VarName2Change])

                FilteredSample = []
                for i in ParamSample:
                    FilteredSample.append([round(val, 3) for val in i])
                ParamSample = np.array(FilteredSample)
                NbRuns2run =[i for i in range(min(NbRuns,len(ParamSample[:,0])))]
