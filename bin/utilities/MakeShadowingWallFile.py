...
def CreatePolygonEnviro(GlobKey,config,WithBackSide = True):
    ...
    MainPath = os.getcwd()
    TotalSimDir = []
    Retrofit_Info = {"RetrofitCase":False}
    for nbfile,keyPath in enumerate(GlobKey):
        ...
        for bldNum, Bld in enumerate(DataBaseInput['Build']):
            ...
            try: BldObj = Building('Bld'+str(bldNum), Retrofit_Info, DataBaseInput, bldNum, SimDir,keyPath['Buildingsfile'],LogFile=[],PlotOnly=True, DebugMode=False)
            except:
...
def getEdgeIdx(Matches,Edge,BldID,BldBloc):
    ...
    if len(edgeIdx)>1:
        print('heu... error in the index caught for the edges done or not... several locations found for a single edge !')
...
def prepareElements(Data,FootKey):
    ...
    for bld in Data[FootKey]:
        ...
        if bld[0]==bld[-1]:
            ...
            NewBld.append(bld)
    #lets compute distances now with starting points from all segments for all polygons and ending point all segment of all polygons except the current one
...
if __name__ == '__main__' :
    MainPath = os.path.dirname(os.getcwd()) #in order to be in the bin folder
    os.chdir(MainPath)
