...
from core import setConfig as setConfig
...
#function to quickly plot polygons for debugin purposes
def Makeplot(poly):
    import matplotlib.pyplot as plt
    x,y = zip(*poly)
    plt.plot(x,y,'.-')

#function to fectch which bloc is above one another
...
#used with formerly made shading wall. no more needed if the shadowing file is made with the specific script in mubes-ubem
...
#find the height of the building's ID
#used with formerly made shading wall. no more needed if the shadowing file is made with the specific script in mubes-ubem
...
class Building:
    def __init__(self,name,DataBaseInput,nbcase,MainPath,BuildingFilePath,LogFile,PlotOnly,DebugMode):
        ...
        self.GE = config['3_SIM']['GeomElement'] #these element might be needed afterward for parametric simulation, thus the key words are needed
    ...
    def CheckAndCorrEPCs(self,Buildingsfile,LogFile,nbcase,EPC,DebugMode):
        #if values are correponding to more than one bvuilding then, it should be split proportionaly to the volume
        totHeat = []
        ...
        for share in tocheck:
            ...
            totHeat.append(val)
        # correction on the DB_SurfBSurf if it is the same on all (should be)
    ...
    def IsSameFormularIdBuilding(self,Buildingsfile,nbcase,LogFile,DBL,DebugMode):
        #function to check if there are some shared EPCs with several buildings and apply some correction is needed
        SharedBld = []
        VolumeCorRatio = 1
        Correction = False
        for nb,Build in enumerate(Buildingsfile):
            try:
                if Build.properties['FormularId'] == Buildingsfile[nbcase].properties['FormularId'] and nb != nbcase:
                    SharedBld.append(nb)
                    Correction = True
            except:
                pass
        ...
        for nb in SharedBld:
            ...
            Bldfootprint,  BldBlocHeight, BldBlocNbFloor, BlocAlt,BlocMaxAlt,BlocAltMatches = self.getfootprint(Buildingsfile[nb],[],floors[-1])
    ...
    def getRefCoord(self):
        ...
        ref = (x-offset, y-offset) #there might be not a true need for such precision... but with particular symatric building, the boundary conditions can fail because of exacte distances form reference point, so an offset is applied
        return ref
    ...
    #before submitting the full coordinates, we need to check correspondance in case of multibloc
        #this check is made after encountering a specific case that appeared only once...
        idx2remove = []
        for idx, poly in enumerate(coord):
            if GeomUtilities.getArea(poly) < 1:
                idx2remove.append(idx)
        ...
        #for energy plus inputs, floor needs to be clockwise oriented
        for poly in coord:
        ...
        BlocAltMatches = getBlocMatches(BlocAlt, BlocMaxAlt,self.AltTolerance)
        BlocHeight = [val-val1 for val,val1 in zip(BlocMaxAlt,BlocAlt)]
        return coord, BlocHeight, BlocNbFloor, BlocAlt,BlocMaxAlt,BlocAltMatches
    ...
    def getIntLoad(self, MainPath,LogFile,DebugMode = False):
        ...
        #we should integrate the loads depending on the number of appartment in the building
        ...
        try:
            IntLoad = self.ElecYearlyLoad   #this value is in W\m2
        except:
            IntLoad = 0
